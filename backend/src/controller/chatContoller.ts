...
export async function startIndividualChat(req: Request, res: Response) {
    ...
    try {
        const existingChat = await pool.query('SELECT chat_id FROM chat_participants WHERE user_id = $1 INTERSECT SELECT chat_id FROM chat_participants WHERE user_id = $2', [senderId, receiverId]);

        if (existingChat.rows.length > 0) {
            return existingChat.rows[0].chat_id;
        }

        const newChat = await pool.query('INSERT INTO chats (name,is_group_chat) values ($1,$2) RETURNING chat_id', ['', false]);
        const chatId = newChat.rows[0].chat_id;

        await pool.query('INSERT INTO chat_participants (chat_id,user_id) values ($1,$2),($1,$3)', [chatId, senderId, receiverId]);
